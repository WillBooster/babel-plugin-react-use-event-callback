...
export default (): PluginObj =>  {
  ...
  const getAllOwnBindings = (scope: Scope) => {
    const allBindings:object = scope.getAllBindings()

    return Object.keys(allBindings).reduce((ownBindings:{}, bindingName: string) => {
  ...
  const getValueExpressions = (parentPath: NodePath<any>) => {
    const values:any[] = []
  ...
  const isAnyFunctionExpression = (node: t.Node) => {
    return node && (t.isArrowFunctionExpression(node) || t.isFunctionExpression(node))
  }

  // If expression ends up with a useXXX()
  const isReactHook = (node: t.Node) => {
    return /^use/.test((<any>node).name) || /^use/.test((<any>node).property.name)
  }
  ...
  const isWrappedWithCreateElement = (path: NodePath) => {
    let currPath = path
    ...
    return (
      ...
      (<any>currPath.node.callee.object).name === 'React' &&
